...
This is an unimpressive MNIST model, but it is a good example of using tf.name_scope
to make a graph legible in the TensorBoard graph explorer, and of naming summary tags
so that they are grouped meaningfully in TensorBoard.
...
def train():
    ...
    def nn_layer(input_tensor, input_dim, output_dim, layer_name, act=tf.nn.relu):
        ...
        """
        # Adding a name scope ensure logical grouping of the layers in the graph.
    ...
    train_writer = tf.summary.FileWriter(FLAGS.log_dir + '/trian', sess.graph)
    ...
    for i in range(FLAGS.max_steps):
        if i % 10 == 0:   # Record summaries and test-set accuracy
            summary, acc = sess.run([merged, accuracy], feed_dict=feed_dict(False))     # False means testing
...
def main(_):
    if tf.gfile.Exists(FLAGS.log_dir):
        print("lod_dir exist: ", FLAGS.log_dir)
...
if __name__ == '__main__':
    ...
    # print(FLAGS)      # FLAGS is an instance of Namespace.
    # print(unparsed)   # [])
    tf.app.run(main=main, argv=[sys.argv[0]] + unparsed)      # sys.argv[0] is the name of the script.
